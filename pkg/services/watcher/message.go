...
func (s *service) handleServerDetails(details ServerGameState) error {
	...
	got, err := redisgo.StringMap(redis.Conn.DoCtx(s.ctx, "HGETALL", "server_details"))
	if err != nil {
	...
	if _, err := redis.Conn.DoCtx(s.ctx, "HMSET", redisgo.Args{}.Add("server_details").AddFlat(values)...); err != nil {
